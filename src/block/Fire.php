...
class Fire extends Flowable
{
	...
	public function onRandomTick(): void
	{
		...
		if ($result !== null) {
			$this->position->getWorld()->setBlock($this->position, $result, false);
		}
		...
		if (mt_rand(0, $chanceBound) < $block->getFlammability()) {
			...
			if (!$ev->isCancelled()) {
				...
				if ($this->position->getWorld()->getBlock($block->getPosition())->isSameState($block)) {
					...
					if (!$spreadedFire) {
						$this->position->getWorld()->setBlock($block->position, VanillaBlocks::AIR(), false);
					}
				}
			}
		}
	...
	private function spreadBlock(Block $block, Block $newState): bool
	{
		...
		$ev->call();
		if (!$ev->isCancelled()) {
			$block->position->getWorld()->setBlock($block->position, $ev->getNewState(), false);
			return true;
		}
